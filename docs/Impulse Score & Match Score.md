...
def compute_impulse_score(
    ...
    risk_score = max(0, min(100, round((raw_score / MAX_POSSIBLE) * 100)))
...
- 데이터 출처
    ...
    - 옷 이미지 묘사 텍스트
        ...
        - 스타일 키워드
            - 예시: 캐주얼, 스트릿, 페미닌, 미니멀, 빈티지, 스포티, 포멀, Y2K, 시티보이
        - 촬영 유형
            - 다음 4개 중에서 고르도록
                - 모델 착용샷 | 흰 배경 단독샷 | 행거샷 | 기타
        - 옷 가시성
            - 다음 3개 중에서 고르도록
                - 양호 | 부분 가림 | 불량
- 점수 계산
    ...
    - 옷 가시성이 "부분 가림"이면 최대 20점을 초과할 수 없다
    - 옷 가시성이 "불량"이면 최대 8점을 초과할 수 없다
