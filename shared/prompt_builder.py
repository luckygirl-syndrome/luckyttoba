"""또바 system_prompt + context_sentences 빌더.

docs/챗봇에게 전달할 system_prompt & context_sentences.md 에서 추출·모듈화.
"""
...
def build_system_prompt(
    ...
    liked_purchases : 만족 구매 스크린샷의 Vision 추출 텍스트 (선택)
    """
    user_axes = list(user_type)

    sbti_context = "\n".join(
        f"- {axis}형: {SBTI_DESCRIPTIONS[axis]}" for axis in user_axes
    )
    ...
    if regret_frequency == "없어요":
        ...
        if regret_reasons:
            profile_lines.append(f"- 주요 후회 이유: {', '.join(regret_reasons)}")
    if liked_purchases:
...
def get_price_full_context(original_price, discounted_price, discount_rate, price_feeling):
    ...
    feeling_part = feeling_mapping.get(price_feeling)
    if feeling_part:
...
def build_context_sentences(
    ...
    price_feeling: str | None = None,
    contact_reason: str | None = None,
    # style_similarity LLM 결과의 reason 문자열
    ...
    Returns
    -------
    {"product_info": [...], "score_context": [...]}
    """
    # product_info
    product_info = [f"상품명: {product_name}"]
